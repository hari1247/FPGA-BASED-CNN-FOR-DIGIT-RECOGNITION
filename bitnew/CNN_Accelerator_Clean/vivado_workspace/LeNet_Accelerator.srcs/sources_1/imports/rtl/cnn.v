  ...
  */  
  ...
  always @(*) begin
    ...
    //======================= Layer 2 ===========================================================================================================
      ...
      L5_OUT:        n_state = (psum_temp_indx == 8) ? DONE : L5_RD_BRTCH1; // Changed from 46 to 8 for MNIST 10-class
  ...
  assign L1_counter_flag = (state == L1_RD_BRTCH1 && counter <= 11) ||
                           (state == L1_RD_BRTCH2 && counter <= 36) || (state == L1_RD_BRTCH3 && counter <= 11) ||
  ...
  assign L2_counter_flag = (state == L2_RD_BRTCH1 && counter <= 35) || (state == L2_RD_BRTCH2 && counter <= 12) ||
                           (state == L2_RD_BRTCH3 && counter <= 35) ||
                           (state == L2_READ12 && counter <= 11) ||
                           (state == L2_MX_PL && counter <= 6) ||
                           (state == L2_EXE && counter <= 2) ||
                           (state == L2_WRITE_TEMP && counter <= 0);
  
  assign L3_counter_flag = (state == L3_RD_BRTCH1 && counter <= 24) ||
                           (state == L3_RD_BRTCH2 && counter <= 23) || (state == L3_RD_BRTCH3 && counter <= 49) ||
                           (state == L3_EXE && counter <= 1);
                           
  assign L4_counter_flag = (state == L4_RD_BRTCH1 && counter <= 29) ||
                           (state == L4_EXE && counter <= 1);
  
  assign L5_counter_flag = (state == L5_RD_BRTCH1 && counter <= 20) ||
                           (state == L5_RD_BRTCH2 && counter <= 20) || (state == L5_EXE && counter <= 1);
...
*/
  ...
  // assign pe_out_sum_a_quan = (mode) ?
  //                                     ((|pe_out_sum_a_relu[31:15]) ? 255 : ((&pe_out_sum_a_relu[14:7]) ? pe_out_sum_a_relu[14:7] : (pe_out_sum_a_relu[14:7] + pe_out_sum_a_relu[6]))) :
  ...
  always @(posedge clk or posedge rst) begin
    if(rst) soft_max_indx <= 0;
      ...
      if(state == L5_OUT) begin
        if(psum_temp_indx == 8) soft_max_indx <= (soft_max_temp > pe_out_sum_a_relu) ? soft_max_indx : psum_temp_indx; // Changed to 8
  ...
  always @(posedge clk or posedge rst) begin
    if(rst) soft_max_temp <= 0;
      ...
      if(state == IDLE) soft_max_temp <= 0;
        ...
        if(psum_temp_indx == 8) soft_max_temp <= (soft_max_temp > pe_out_sum_a_relu) ? soft_max_temp : pe_out_sum_a_relu; // Changed to 8
